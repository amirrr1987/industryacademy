...
//add_theme_support( 'wc-product-gallery-slider' );
	...
	add_image_size('studiare-image-250x400-croped', 250, 400, true);
...
if ( ! function_exists( 'studiare_init') ) {
	...
	function studiare_init() {
		...
		//if(is_rtl()){
	//	wp_register_style( 'studiare-rtl', get_template_directory_uri().'/rtl.css' , null, $theme_version );
	//	}
		if ( class_exists( 'WooCommerce' ) ) {
		wp_register_style( 'studiare-woo', get_theme_file_uri( 'assets/css/woocommerce.css' ), null, $theme_version );
		}
		...
		wp_enqueue_style( 'font-awesome-css', get_theme_file_uri( 'assets/css/fonawesomeall.min.css' ) );
		//wp_enqueue_style( 'material-icons', get_theme_file_uri( 'assets/css/material-icons.css' ) );

		wp_enqueue_style( 'sc-min-hint', get_theme_file_uri( 'assets/css/sc-min-hint.css' ) );
        wp_enqueue_style( 'toastr', get_theme_file_uri( 'assets/css/toastr.css' ) );
...
if ( ! function_exists( 'studiare_wp_enqueue_scripts' ) ) {
	...
	function studiare_wp_enqueue_scripts() {
		...
		wp_enqueue_style( array( 'studiare-rtl') );
		wp_enqueue_style( array( 'studiare-woo') );
        ...
        wp_enqueue_script( 'studiare-theme', get_theme_file_uri( 'assets/js/global.js' ), array( 'jquery' ), '',true );
				wp_enqueue_script( 'toastr', get_theme_file_uri( 'assets/js/vendor/toastr.js' ), array( 'jquery' ), '',true );
...
function studiare_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here to appear in your sidebar on blog archive pages.', 'studiare' ),
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Blog Single Sidebar', 'studiare' ),
			'id'            => 'sidebar-2',
			'description'   => esc_html__( 'Add widgets here to appear in your sidebar on single blog posts.', 'studiare' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h5 class="widget-title">',
			'after_title'   => '</h5>',
	...
	if ( class_exists( 'WooCommerce' ) ) {
		register_sidebar( array(
			'id'            => 'singleshop',
			'name'          => esc_html__('Single Shop Sidebar', 'studiare'),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h5 class="widget-title">',
			'after_title'   => '</h5>',
		        ...
		        if ( class_exists('Redux' ) ) {
			        ...
			        ?>
                        ...
                        <div style="text-align:center">
                            ...
                            <div>
                            <?php if(!empty(codebean_option('custom_preloader_image')['thumbnail'])){ 
                              $img_login_src = codebean_option('custom_preloader_image')['url'];
                              }
                              else{
                                $img_login_src = get_template_directory_uri()."/assets/images/logo_default.svg";
                              }
                            ?>
                         <img <?php list($width, $height) = getimagesize($img_login_src);?> width ="<?php echo $width;?>" height ="<?php echo $height;?>" src="<?php echo $img_login_src;?>"></div><h1 style=" text-align: center; width: 100%; border-top: 1px solid; margin-top: 11px; padding-top: 10px; "><?php echo get_bloginfo( 'name' ); ?></h1>
...
add_action( 'wp_head', 'studiare_favicon',999999999999 );
add_action( 'admin_head', 'studiare_favicon',0 );
