...
<div class="entry-register">
	...
	<?php } else { ?>
        ...
        $event_login_to_woo = false;
        if ( class_exists('Redux') ) {
    		$event_login_to_woo = codebean_option('event_login_to_woo');
    		
	    }
	    if($event_login_to_woo){
	        if (class_exists('WooCommerce')) {

              echo  sprintf( __( 'You must <a href="%s">login</a> before register event.', 'wp-events-manager' ), esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) ); 
            }else{        
                echo sprintf( __( 'You must <a href="%s">login</a> before register event.', 'wp-events-manager' ), wpems_login_url() ); 
                    }
